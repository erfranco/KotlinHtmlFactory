...
const val AUTHOR = " Ermanno Franco"
const val EMAIL = "ermanno.franco@gmail.com"
const val SPACES_PER_TAB = 2
const val AJAX_GOOGLE_LIBS = "https://ajax.googleapis.com/ajax/libs/"
...
object StringUtils {
    ...
    private fun encode(source: String, encodingTable: HashMap<Char, String>): String {
        //var src: String
        val encoded_string = StringBuilder()
        val string_to_encode_array = source.toCharArray()
        var last_match = -1
        var difference: Int
        string_to_encode_array.indices.forEach {
            val char_to_encode = string_to_encode_array[it]
            if (encodingTable.containsKey(char_to_encode)) {
                difference = it - (last_match + 1)
                if (difference > 0) {
        ...
        return if (encoded_string.isEmpty()) {
            source
        } else {
            ...
            val src = encoded_string.toString().replace("\r", "")
            .replace("\t", Ents.BLANK.entity.repeat(SPACES_PER_TAB))
            .replace("\n", "<br />\r\n")
            src
        }
    }
